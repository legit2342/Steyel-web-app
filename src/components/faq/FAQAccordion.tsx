...
export default function FAQAccordion() {
  ...
  return (
    <section className="bg-[#0b0a0f] px-6 pb-24">
      <Reveal className="mx-auto max-w-3xl rounded-[32px] border border-white/10 bg-white/[0.02] p-6 sm:p-10">
        <div className="relative aspect-[688/172] w-full overflow-hidden rounded-2xl">
          <Image src="/images/faq/banner-wave.png" alt="" fill className="object-cover" />
        </div>
        ...
        <div className="mt-8 flex flex-col gap-4">
          {QUESTIONS.map((item, i) => {
            ...
            return (
              <div
                key={item.q}
                className={`overflow-hidden rounded-2xl border border-white/10 transition-all duration-300 ${
              ...
              >
                ...
                >
                  <p className="overflow-hidden px-6 pb-5 text-sm leading-relaxed text-white/50">{item.a}</p>
                </div>
              </div>
            );
          })}
